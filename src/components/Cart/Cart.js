...
const Cart = () => {
                ...
                <div className="row mt-5">
                    <div className="col-md-8">
                        <div className="p-3 pt-0">
                            <p className="bold m-0"> Cart <span className="hidden">({numOfItem} items)</span></p>
                        </div>
                        {/* {CartItems.map(p => <CartItem key={p.id} product={p} />)} */}
                        {state.map(p => <CartItem 
                        key={p.id} 
                        product={p}  
                        ...
                        <div className="my-4 text-center">
                            <NavLink className="btn btn-primary text-white " to="/Home/Category">Continue Shopping</NavLink>
                        </div>
                    </div>
    ...
    const pricArr = state.map(p => +p.price * p.quan); 
